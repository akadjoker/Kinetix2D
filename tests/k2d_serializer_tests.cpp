...
bool NearVec2(const Math::Vec2& a, const Math::Vec2& b)
{
...
bool TestRoundTripThroughText()
{
    ...
    rootSprite->setWaterEnabled(true);
    rootSprite->water().strength = 0.031f;
    rootSprite->water().normalScale = 5.0f;
    rootSprite->water().flowA = {0.12f, -0.03f};
    rootSprite->water().flowB = {-0.08f, 0.09f};
    rootSprite->water().tint = k2d::Color(0.65f, 0.82f, 1.0f, 0.9f);
    rootSprite->water().highlight = 0.42f;
    ...
    if (copySprite)
    {
        ...
        ok = ok && copySprite->waterEnabled();
        ok = ok && Near(copySprite->water().strength, 0.031f) && Near(copySprite->water().normalScale, 5.0f) &&
             NearVec2(copySprite->water().flowA, {0.12f, -0.03f}) &&
             NearVec2(copySprite->water().flowB, {-0.08f, 0.09f}) &&
             NearColor(copySprite->water().tint, k2d::Color(0.65f, 0.82f, 1.0f, 0.9f)) &&
             Near(copySprite->water().highlight, 0.42f);
    }
...
bool TestDefaultsRoundTrip()
{
    ...
    return copy->active() && copy->visible() && NearVec2(copy->scale(), {1.0f, 1.0f}) && copy->childCount() == 0 &&
           copy->getComponent<k2d::SpriteComponent>() == nullptr;
}
...
bool TestUnregisteredTypeIsSkipped()
{
    ...
    return k2d::Serializer::IsRegistered(k2d::ComponentType::Sprite) && components.is_array() && components.size() == 1;
}
...
bool TestTileMapRoundTrip()
{
    ...
    ok = ok && NearVec2(copyMap->atlasPadding(), {2.0f, 3.0f}) && NearVec2(copyMap->atlasGap(), {1.0f, 2.0f});
    ok = ok && copyMap->getTile(0, 0) == 3 && copyMap->getTile(2, 1) == 7 && copyMap->getTile(1, 0) == 0;
...
bool TestPolygonAndLineRoundTrip()
{
    ...
    if (copyPolygon)
    {
        ok = ok && copyPolygon->polygon().size() == 3 && NearVec2(copyPolygon->polygon()[2], {5.0f, 10.0f});
    ...
    if (copy->childCount() == 1)
    {
        ...
        if (copyLine)
        {
            ok = ok && copyLine->points().size() == 3 && copyLine->closed() == true && Near(copyLine->width(), 6.0f) &&
                 copyLine->color() == line->color() && copyLine->blendMode() == k2d::BLEND_ADD;
        }
    }
    return ok;
}
...
bool TestPrimitiveShapeRoundTrip()
{
    ...
    return Near(copyCircle->radius(), 26.0f) && copyCircle->segments() == 18 &&
           copyCircle->mode() == k2d::ShapeRenderMode::Line && Near(copyCircle->lineWidth(), 3.5f) &&
           copyCircle->color() == circle->color() && copyCircle->blendMode() == k2d::BLEND_ADD &&
           NearVec2(copyRect->size(), {90.0f, 40.0f}) && copyRect->mode() == k2d::ShapeRenderMode::Line &&
           Near(copyRect->lineWidth(), 5.0f) && copyRect->color() == rect->color() &&
           copyRect->blendMode() == k2d::BLEND_MUL && NearVec2(copyCapsule->size(), {140.0f, 48.0f}) &&
           copyCapsule->segments() == 12 && copyCapsule->mode() == k2d::ShapeRenderMode::Line &&
...
bool TestNinePatchRoundTrip()
{
    ...
    return NearVec2(copyPatch->size(), {120.0f, 80.0f}) && NearVec4(copyPatch->margins(), {4.0f, 6.0f, 8.0f, 10.0f}) &&
...
bool TestSpriteBatchRoundTrip()
{
    ...
    return e0 && NearVec2(e0->position, {1.0f, 2.0f}) && NearVec2(e0->size, {30.0f, 40.0f}) &&
           ...
           (e0->flags & 0x1u) != 0 && (e0->flags & 0x2u) == 0 && copyBatch->blendMode() == k2d::BLEND_ADD;
}
...
bool TestAnimationRoundTrip()
{
    ...
    ok = ok && run && NearVec2(run->atlasPadding, {2.0f, 3.0f}) && NearVec2(run->atlasGap, {1.0f, 2.0f});
    ...
    ok = ok && copyAnim->frameCount("effect") == 2 && effectFrame &&
         NearVec4(effectFrame->rect, {14.0f, 4.0f, 10.0f, 12.0f}) && NearVec2(effectFrame->offset, {3.0f, -2.0f});
    return ok;
}
...
bool TestLightDisambiguation()
{
    ...
    ok = ok && copyPoint->castShadow() == true && copyPoint->cullMask() == 3u && Near(copyPoint->height(), 12.0f);
...
bool TestOccluderRoundTrip()
{
    ...
    return copyOccluder && copyOccluder->points().size() == 4 && NearVec2(copyOccluder->points()[2], {40.0f, 10.0f});
}
...
bool TestParticleComponentRoundTrip()
{
    ...
    ok = ok && copyPc->blendMode() == k2d::BLEND_ADD && copyPc->ySort() == true && copyPc->followOwner() == false;
