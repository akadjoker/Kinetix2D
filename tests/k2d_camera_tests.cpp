...
int main()
{
    ...
    camera.camera().position = glm::vec2(640.0f, 360.0f);

    glm::vec2 center = camera.screenToWorld(640.0f, 360.0f);
    ...
    camera.camera().zoom = glm::vec2(2.0f);
    camera.setViewport(800.0f, 600.0f);
    camera.camera().position = glm::vec2(400.0f, 300.0f);
    camera.visibleRect(minX, minY, maxX, maxY);
    ...
    follow.setTarget(glm::vec2(1900.0f, 900.0f));
    ...
    smooth.setTarget(glm::vec2(100.0f, 0.0f));
    ...
    deadZone.setTarget(glm::vec2(200.0f, 200.0f));
