...
int main()
{
    ...
    kx::Body *dynamic = world.CreateCircle(Math::Vec2(10.0f, 0.0f), 5.0f, 1.0f);
    kx::Body *staticBody = world.CreateStaticBox(Math::Vec2(100.0f, 0.0f), 5.0f, 5.0f);
    kx::Body *kinematic = world.CreateKinematicBox(Math::Vec2(-100.0f, 0.0f), 5.0f, 5.0f);

    ct::Vector<kx::Body *> hits;
    kx::AABB box;
    box.lowerBound = Math::Vec2(0.0f, -8.0f);
    box.upperBound = Math::Vec2(20.0f, 8.0f);
    world.QueryAABB(box, hits);
    Check(hits.size() == 1 && hits[0] == dynamic, "QueryAABB returns overlapping body");

    world.QueryCircle(Math::Vec2(0.0f), 20.0f, hits);
    Check(hits.size() == 1 && hits[0] == dynamic, "QueryCircle returns body in radius");

    kx::Explode(world, Math::Vec2(0.0f), 20.0f, 100.0f, 1.0f);
