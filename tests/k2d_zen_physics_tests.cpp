...
static bool testBodyHandleDrivesTheSimulation()
{
    ...
    k2d::Scene scene;
    k2d::GameObject *object = makeBox(scene, "rocket", Math::Vec2(0.0f, 0.0f),
    ...
    std::printf("  body_handle: x=%.1f vx=%g\n", object->position().x,
                k2d::ZenBlackboard::getNumber("vx"));
    return ok;
}
...
static bool testImpulseFromScript()
{
    ...
    k2d::Scene scene;
    k2d::GameObject *object = makeBox(scene, "jumper", Math::Vec2(0.0f, 0.0f),
                                      Math::Vec2(40.0f, 40.0f), kx::BodyType::Dynamic);
    ...
    std::printf("  impulse: vy=%.2f (impulse/mass = %.2f) y=%.1f\n",
                body ? body->velocity().y : 0.0f, expected, object->position().y);
    return ok;
}
...
static bool testCollisionCallbackReachesScripts()
{
    ...
    makeBox(scene, "floor", Math::Vec2(0.0f, 300.0f), Math::Vec2(600.0f, 40.0f),
            kx::BodyType::Static);
    k2d::GameObject *faller = makeBox(scene, "faller", Math::Vec2(0.0f, 0.0f),
                                      Math::Vec2(40.0f, 40.0f), kx::BodyType::Dynamic);
...
static bool testSensorReportsToScript()
{
    ...
    makeBox(scene, "floor", Math::Vec2(0.0f, 500.0f), Math::Vec2(600.0f, 40.0f),
            kx::BodyType::Static);
    makeBox(scene, "diver", Math::Vec2(0.0f, 0.0f), Math::Vec2(40.0f, 40.0f),
            kx::BodyType::Dynamic);
...
static bool testRaycastAndGravityFromScript()
{
    ...
    k2d::Scene scene;
    makeBox(scene, "wall", Math::Vec2(200.0f, 0.0f), Math::Vec2(40.0f, 200.0f),
            kx::BodyType::Static);

    k2d::GameObject *scanner = scene.createObject("scanner");
    scanner->addComponent<k2d::ZenScriptComponent>()->loadSource(
        "class Scanner:\n"
        "    def __init__(self, node):\n"
        "        self.node = node\n"
        "\n"
...
int main()
{
    ...
    std::printf("zen physics: body_handle=%s impulse=%s on_collision=%s sensor=%s queries=%s "
                "no_body=%s\n",
                handle ? "pass" : "fail", impulse ? "pass" : "fail",
                collision ? "pass" : "fail", sensor ? "pass" : "fail",
                queries ? "pass" : "fail", noBody ? "pass" : "fail");
    return handle && impulse && collision && sensor && queries && noBody ? 0 : 1;
}
