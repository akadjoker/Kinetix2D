...
namespace k2d
{
    ...
    namespace
    {
        ...
        const char *bodyTypeName(kx::BodyType type)
        {
            switch (type)
            {
            case kx::BodyType::Static: return "static";
            case kx::BodyType::Kinematic: return "kinematic";
        ...
        void readShared(Collider2D &collider, const ct::Json &data)
        {
            ...
            collider.setFilter((uint16_t)data["category"].as_int(1),
                               (uint16_t)data["mask"].as_int(0xFFFF));
        }

        Component *createBox(GameObject &owner) { return owner.addComponent<BoxCollider2D>(); }
        ...
        Component *createCircle(GameObject &owner) { return owner.addComponent<CircleCollider2D>(); }
        ...
        Component *createEdge(GameObject &owner) { return owner.addComponent<EdgeCollider2D>(); }
        ...
        void readEdge(Component &component, const ct::Json &data, Assets *)
        {
            ...
            edge.setPoints(readVec2(data["start"], Math::Vec2(-32.0f, 0.0f)),
                           readVec2(data["end"], Math::Vec2(32.0f, 0.0f)));
        }
        ...
        Component *createChain(GameObject &owner) { return owner.addComponent<ChainCollider2D>(); }
    ...
    {
        Serializer::RegisterType(ComponentType::RigidBody, "RigidBody2D", &createRigidBody,
                                 &writeRigidBody, &readRigidBody, &isRigidBody);
        Serializer::RegisterType(ComponentType::Collider, "BoxCollider2D", &createBox, &writeBox,
                                 &readBox, &isBox);
        Serializer::RegisterType(ComponentType::Collider, "CircleCollider2D", &createCircle,
                                 &writeCircle, &readCircle, &isCircle);
        Serializer::RegisterType(ComponentType::Collider, "EdgeCollider2D", &createEdge, &writeEdge,
                                 &readEdge, &isEdge);
        Serializer::RegisterType(ComponentType::Collider, "PolygonCollider2D", &createPolygon,
                                 &writePolygon, &readPolygon, &isPolygon);
        Serializer::RegisterType(ComponentType::Collider, "ChainCollider2D", &createChain,
                                 &writeChain, &readChain, &isChain);
    }
