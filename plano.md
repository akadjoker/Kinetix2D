...
- [x] Criar uma atualização conjunta de posição e rotação para a sincronização física.
- [x] Fazer uma única invalidação de transform por corpo.
...
`pullTransforms()` agora usa `GameObject::setPositionAndRotation()`, reduzindo duas invalidações de subárvore para uma. No benchmark de 1.000 corpos e 3.783 contatos, `physics2d_step_avg_ms` caiu de `51.2279` para `48.0993` ms, ganho de 6,11%. A suíte ampla passou 31/31 testes.
...
## Web exporter e scripts

- [x] Limitar a descoberta de scripts a `project/assets/scripts/**/*.py`.
- [x] Não criar bundle de scripts quando `assets/scripts` não existir ou estiver vazio.
- [x] Remover `scripts.zbc` e `scripts.json` antigos antes de cada export para evitar carregar scripts obsoletos.
- [x] Manter os caminhos do manifest relativos à raiz do projeto, compatíveis com o runner.
- [x] Validar a sintaxe com `python3 -m py_compile tools/export_web.py`.
- [ ] Validar export completo em ambiente com Emscripten e projeto Web de teste.

O problema observado de scripts aparentemente não carregarem podia vir de artefatos antigos em `.k2d/web` ou de scripts fora da pasta esperada. O exporter agora usa somente `assets/scripts` e limpa os bundles anteriores. O export Web completo ainda não foi executado neste ambiente porque não há Emscripten nem projeto Web de teste disponível.
