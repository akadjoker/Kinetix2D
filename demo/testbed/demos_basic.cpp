...
{
public:
    explicit SingleBox(Testbed &tb) : Demo(tb, Math::Vec2(0.0f, -300.0f))
    {
        T().Cam().center = Math::Vec2(0.0f, 60.0f);

        World().CreateEdge(Math::Vec2(-600.0f, 0.0f), Math::Vec2(600.0f, 0.0f));

        kx::Body *box = World().CreateBox(Math::Vec2(0.0f, 40.0f), 20.0f, 20.0f, 1.0f);
        box->SetVelocity(Math::Vec2(150.0f, 0.0f));
        mBox = box;
    }
...
{
public:
    explicit PyramidStack(Testbed &tb) : Demo(tb, Math::Vec2(0.0f, -100.0f))
    {
        ...
        for (int i = 0; i < 14; ++i)
        {
            for (int j = 0; j <= i; ++j)
            {
                kx::Body *box = World().CreateBox(Math::Vec2(j * 32.0f - i * 16.0f, 300.0f - i * 32.0f), 15.0f, 15.0f, 1.0f);
                box->SetFriction(0.8f);
            }
        }

        kx::Body *ball = World().CreateCircle(Math::Vec2(0.0f, -220.0f), 15.0f, 1.0f);
...
{
public:
    explicit TiltedStack(Testbed &tb) : Demo(tb, Math::Vec2(0.0f, -300.0f))
    {
        T().Cam().center = Math::Vec2(0.0f, 250.0f);
        T().Cam().zoom = 1.0f;

        World().CreateStaticBox(Math::Vec2(0.0f, -20.0f), 400.0f, 20.0f);
        ...
        for (int j = 0; j < kColumns; ++j)
        {
            ...
            {
                kx::Body *box = World().CreateBox(Math::Vec2(x + offset * i, 15.0f + 30.0f * i), 13.5f, 13.5f, 1.0f);
...
{
public:
    explicit Plink(Testbed &tb) : Demo(tb, Math::Vec2(0.0f, -100.0f))
    {
        ...
        Math::Vec2 tris[3] = {
            Math::Vec2(-15.0f, -15.0f),
            Math::Vec2(0.0f, 10.0f),
            Math::Vec2(15.0f, -15.0f)};
        ...
        for (int i = 0; i < 9; ++i)
        {
            for (int j = 0; j < 6; ++j)
            {
                ...
                Math::Vec2 pos(i * 80.0f - 320.0f + stagger, j * 70.0f - 240.0f);
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            verts[i] = Math::Vec2(10.0f * cosf(angle), 10.0f * sinf(angle));
        }

        for (int i = 0; i < 300; ++i)
        {
            float x = Rnd() * 640.0f - 320.0f;
            kx::Body *penta = World().CreateBody(kx::BodyType::Dynamic, Math::Vec2(x, 350.0f));
    ...
    void Step(float) override
    {
        for (size_t i = 0; i < mPentagons.size(); ++i)
        {
            ...
            Math::Vec2 pos = body->Position();
            if (pos.y < -260.0f || pos.x > 340.0f || pos.x < -340.0f)
            {
                body->SetPosition(Math::Vec2(Rnd() * 640.0f - 320.0f, 260.0f));
...
{
public:
    explicit Tumble(Testbed &tb) : Demo(tb, Math::Vec2(0.0f, -600.0f))
    {
        T().Cam().center = Math::Vec2(0.0f, 0.0f);

        mBox = World().CreateBody(kx::BodyType::Kinematic, Math::Vec2(0.0f, 0.0f));
        mBox->AddEdge(Math::Vec2(-200.0f, -200.0f), Math::Vec2(-200.0f, 200.0f));
        mBox->AddEdge(Math::Vec2(-200.0f, 200.0f), Math::Vec2(200.0f, 200.0f));
        mBox->AddEdge(Math::Vec2(200.0f, 200.0f), Math::Vec2(200.0f, -200.0f));
        mBox->AddEdge(Math::Vec2(200.0f, -200.0f), Math::Vec2(-200.0f, -200.0f));
        ...
        for (int i = 0; i < 7; ++i)
        {
            for (int j = 0; j < 3; ++j)
            {
                Math::Vec2 pos(i * 30.0f - 150.0f, j * 60.0f - 150.0f);
                ...
                {
                    kx::Body *c1 = World().CreateCircle(pos + Math::Vec2(0.0f, 15.0f), 15.0f, 1.0f);
                    c1->SetFriction(0.7f);
                    kx::Body *c2 = World().CreateCircle(pos + Math::Vec2(0.0f, -15.0f), 15.0f, 1.0f);
                    c2->SetFriction(0.7f);
                }
            }
        }
