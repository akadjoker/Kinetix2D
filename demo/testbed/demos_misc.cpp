...
{
public:
    explicit Planet(Testbed &tb) : Demo(tb, glm::vec2(0.0f, 0.0f))
    {
        T().Cam().center = glm::vec2(0.0f, 0.0f);

        mPlanet = World().CreateBody(kx::BodyType::Kinematic, glm::vec2(0.0f, 0.0f));
        mPlanet->AddCircle(glm::vec2(0.0f, 0.0f), 70.0f, 1.0f);
    ...
    void Step(float dt) override
    {
        ...
        for (size_t i = 0; i < mBoxes.size(); ++i)
        {
            ...
            glm::vec2 g = p * (-kGravityStrength / (sqdist * sqrtf(sqdist)));
...
private:
    void AddBox()
    {
        ...
        do
        {
            pos = glm::vec2(Rnd() * 640.0f - 320.0f, Rnd() * 480.0f - 240.0f);
        ...
        body->AddBox(10.0f, 10.0f, glm::vec2(0.0f, 0.0f), 1.0f);
        body->SetVelocity(glm::vec2(-pos.y, pos.x) * v);
...
{
public:
    explicit ImageShape(Testbed &tb) : Demo(tb, glm::vec2(0.0f, -500.0f))
    {
        T().Cam().center = glm::vec2(0.0f, 40.0f);
        T().SetStatus("SPACE spawns a bunny body traced from its sprite silhouette");

        World().CreateStaticBox(glm::vec2(0.0f, -170.0f), 500.0f, 10.0f);
...
private:
    void Spawn(float x)
    {
        if (!mPixels)
            return;
        World().CreateFromImage(glm::vec2(x, 240.0f), mPixels, mWidth, mHeight, 4, 128, 1.0f, 2.0f);
    }
