...
class CanvasRenderer
{
  public:
    ...
    void DrawItems(const RenderItem* items, size_t count, const PointLight* lights, size_t lightCount,
                   const DirectionalLight* directionalLights, size_t directionalLightCount, const Occluder* occluders,
                   size_t occluderCount);
    ...
    void setDefaultLightTexture(unsigned int textureId)
    {
        SetDefaultLightTexture(textureId);
    }
    void setCanvasModulate(float r, float g, float b)
    {
        SetCanvasModulate(r, g, b);
    }
    unsigned int createShader(const char* fragmentSource)
    {
        return CreateShader(fragmentSource);
    }
    void destroyShader(unsigned int program)
    {
    ...
    void drawItems(const RenderItem* items, size_t count, const PointLight* lights, size_t lightCount,
                   ...
                   size_t occluderCount)
    {
        DrawItems(items, count, lights, lightCount, directionalLights, directionalLightCount, occluders, occluderCount);
    }
    void resetStats()
    {
        ResetStats();
...
#pragma pack(pop)
    ...
    void EmitQuad(BlendMode blendMode, unsigned int textureId, unsigned int normalTextureId, unsigned int customProgram,
                  const WaterEffect& water, const Matrix2D& matrix, float width, float height, int texWidth,
                  int texHeight, float pivotX, float pivotY, float srcX, float srcY, float srcW, float srcH, bool flipX,
                  bool flipY, const Color& color, unsigned int lightMask);
    void EmitPolygon(BlendMode blendMode, unsigned int textureId, unsigned int normalTextureId,
                     ...
                     const ct::Vector<Math::Vec2>& points, const Color& color, unsigned int lightMask, int texWidth = 0,
                     int texHeight = 0);
    void EmitText(BlendMode blendMode, const Matrix2D& matrix, float x, float y, float size, const char* text,
