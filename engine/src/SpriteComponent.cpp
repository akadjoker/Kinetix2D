...
void SpriteComponent::onRender(RenderQueue& queue)
{
    ...
    RenderCommand rect =
        RenderCommand::MakeRect(mMaterial.texture()->Id(), mRenderOffset.x, mRenderOffset.y, mSize.x, mSize.y);
    ...
    rect.flags = (unsigned char)((mMaterial.flipX() ? 1 : 0) | (mMaterial.flipY() ? 2 : 0));
