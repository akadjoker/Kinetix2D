...
BatchRenderer::BatchRenderer()
    : mCurrentMatrix(1.0f), mCurrentColor(0xFFFFFFFF), mCurrentTextureId(0), mCurrentMode(MODE_TRIANGLES),
      mInBeginEnd(false), mCurrentBlendMode(BLEND_ALPHA), mWindowWidth(800), mWindowHeight(600), mProjection(1.0f),
      mDepthTestEnabled(false), mDepthWriteEnabled(false), mBlendEnabled(true), mCullFaceEnabled(false),
      mClipEnabled(false), mVAO(0), mVBO(0), mIBO(0), mProgram(0), mWhiteTexture(0), mFontTexture(0), mMvpLoc(-1),
      mTexLoc(-1), mFrameStartTime(0)
{
...
{
    mCurrentColor = PackColor((unsigned char)(r * 255.0f + 0.5f), (unsigned char)(g * 255.0f + 0.5f),
...
void BatchRenderer::EmitTexturedTriangle(float x0, float y0, float u0, float v0, float x1, float y1, float u1, float v1,
...
void BatchRenderer::Begin(int mode)
{
    ...
    if (mDrawCalls.empty() || mDrawCalls.back().mode != mode || mDrawCalls.back().textureId != mCurrentTextureId ||
        mDrawCalls.back().depthTest != mDepthTestEnabled || mDrawCalls.back().depthWrite != mDepthWriteEnabled ||
        mDrawCalls.back().blend != mBlendEnabled || mDrawCalls.back().blendMode != mCurrentBlendMode ||
...
void BatchRenderer::DrawRect(float x, float y, float width, float height, bool fill)
{
    ...
    else
    {
        ...
        if (mClipEnabled)
        {
            float corners[4][2] = {{x, y}, {x + width, y}, {x + width, y + height}, {x, y + height}};
            for (int i = 0; i < 4; ++i)
            {
...
void BatchRenderer::DrawEllipse(float cx, float cy, float rx, float ry, bool fill, int segments)
{
    ...
    if (fill)
    {
        ...
        for (int i = 0; i < segments; ++i)
        {
            ...
            EmitTriangle(cx, cy, cx + rx * cosf(a0), cy + ry * sinf(a0), cx + rx * cosf(a1), cy + ry * sinf(a1));
        }
        End();
    }
    else
    {
        ...
        {
            float a0 = i * step, a1 = (i + 1) * step;
            EmitLine(cx + rx * cosf(a0), cy + ry * sinf(a0), cx + rx * cosf(a1), cy + ry * sinf(a1));
        }
        End();
    }
}
...
void BatchRenderer::DrawArc(float cx, float cy, float radius, float startDeg, float endDeg, int segments)
{
    ...
    for (int i = 0; i < segments; ++i)
    {
        ...
        EmitLine(cx + radius * cosf(t0), cy + radius * sinf(t0), cx + radius * cosf(t1), cy + radius * sinf(t1));
    }
    End();
}
...
void BatchRenderer::DrawPolygon(float cx, float cy, int sides, float radius, float rotDeg, bool fill)
{
    ...
    if (fill)
    {
        ...
        for (int i = 0; i < sides; ++i)
        {
            ...
            EmitTriangle(cx, cy, cx + radius * cosf(a0), cy + radius * sinf(a0), cx + radius * cosf(a1),
                         cy + radius * sinf(a1));
        }
        End();
    }
    else
    {
        ...
        {
            float a0 = rot + i * step, a1 = rot + (i + 1) * step;
            EmitLine(cx + radius * cosf(a0), cy + radius * sinf(a0), cx + radius * cosf(a1), cy + radius * sinf(a1));
        }
        End();
    }
}
...
void BatchRenderer::DrawTexture(unsigned int textureId, float dstX, float dstY, float dstW, float dstH, float srcX,
...
void BatchRenderer::DrawTexture(unsigned int textureId, const Matrix2D& matrix, float width, float height, int texWidth,
                                int texHeight, float pivotX, float pivotY, float srcX, float srcY, float srcW,
                                float srcH, bool flipX, bool flipY)
{
    if (mVertices.size() + 4 >= mConfig.maxVertices)
        FlushBatch();
...
void BatchRenderer::UnpackColor(unsigned int packed, unsigned char& r, unsigned char& g, unsigned char& b,
                                unsigned char& a)
{
    r = packed & 0xFF;
    g = (packed >> 8) & 0xFF;
...
{
    mProjection = Math::Mat4::Ortho(0.0f, (float)mWindowWidth, (float)mWindowHeight, 0.0f, -1.0f, 1.0f);
}
...
void BatchRenderer::ApplyDrawCalls()
{
    ...
    for (size_t i = 0; i < mDrawCalls.size(); ++i)
    {
        ...
        if (call.blend)
        {
            ...
            {
            case BLEND_ADDITIVE:
                mState.SetBlendFunc(GL_SRC_ALPHA, GL_ONE);
                break;
            case BLEND_MULTIPLIED:
                mState.SetBlendFunc(GL_DST_COLOR, GL_ZERO);
                break;
            case BLEND_ADD_COLORS:
                mState.SetBlendFunc(GL_SRC_COLOR, GL_DST_COLOR);
                break;
            case BLEND_SUBTRACT_COLORS:
                mState.SetBlendFunc(GL_SRC_COLOR, GL_ONE_MINUS_DST_COLOR);
                break;
            case BLEND_ALPHA:
            default:
                mState.SetBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
                break;
            }
        }
...
void BatchRenderer::DrawText(float x, float y, float size, const char* text)
{
    ...
    for (const char* c = text; *c; ++c)
    {
        ...
        if (code != ' ')
        {
            ...
            EmitTexturedTriangle(penX + size, penY, u1, v0, penX + size, penY + size, u1, v1, penX, penY + size, u0,
                                 v1);
        }
        penX += size;
    }
