...
Math::Vec4 ReadVec4(const ct::Json& j, Math::Vec4 def = Math::Vec4(0.0f))
{
    if (!j.is_array() || j.size() < 4)
        return def;
    return Math::Vec4((float)j[0].as_double(def.x), (float)j[1].as_double(def.y), (float)j[2].as_double(def.z),
                      (float)j[3].as_double(def.w));
}
...
Color ReadColor(const ct::Json& j, Color def = Color(1.0f))
{
    if (!j.is_array() || j.size() < 4)
        return def;
    return Color((float)j[0].as_double(def.r), (float)j[1].as_double(def.g), (float)j[2].as_double(def.b),
                 (float)j[3].as_double(def.a));
}
...
void ReadPackedColorBytes(const ct::Json& j, unsigned char& r, unsigned char& g, unsigned char& b, unsigned char& a)
{
...
void WriteSprite(const Component& component, ct::Json& data, Assets* assets)
{
    ...
    const WaterEffect& water = material.water();
    if (water.enabled)
    {
        ct::Json waterData = ct::Json::object();
        waterData.set("strength", ct::Json((double)water.strength));
        waterData.set("normalScale", ct::Json((double)water.normalScale));
        waterData.set("flowA", WriteVec2(water.flowA));
        waterData.set("flowB", WriteVec2(water.flowB));
        waterData.set("tint", WriteColor(water.tint));
        waterData.set("highlight", ct::Json((double)water.highlight));
        data.set("water", waterData);
    }
}
...
void ReadSprite(Component& component, const ct::Json& data, Assets* assets)
{
    ...
    sprite.setColor((unsigned char)std::lround(color.r * 255.0f), (unsigned char)std::lround(color.g * 255.0f),
                    (unsigned char)std::lround(color.b * 255.0f), (unsigned char)std::lround(color.a * 255.0f));
    ...
    if (const ct::Json* waterData = data.find("water"))
    {
        WaterEffect& water = sprite.water();
        water.enabled = true;
        water.strength = (float)(*waterData)["strength"].as_double(water.strength);
        water.normalScale = (float)(*waterData)["normalScale"].as_double(water.normalScale);
        water.flowA = ReadVec2((*waterData)["flowA"], water.flowA);
        water.flowB = ReadVec2((*waterData)["flowB"], water.flowB);
        water.tint = ReadColor((*waterData)["tint"], water.tint);
        water.highlight = (float)(*waterData)["highlight"].as_double(water.highlight);
    }
}
...
void ReadTileMap(Component& component, const ct::Json& data, Assets* assets)
{
    ...
    tileMap.setCellSize((float)data["cellWidth"].as_double(32.0), (float)data["cellHeight"].as_double(32.0));
...
Component* CreateNavigationRegion(GameObject& owner)
{
...
void WriteNavigationRegion(const Component& component, ct::Json& data, Assets*)
{
    const NavigationRegion2D& region = static_cast<const NavigationRegion2D&>(component);
    ct::Json points = ct::Json::array();
...
void WriteNavigationAgent(const Component& component, ct::Json& data, Assets*)
{
    ...
    data.set("autoMove", ct::Json(agent.autoMove()));
...
{
    NavigationAgent2D& agent = static_cast<NavigationAgent2D&>(component);
    agent.setPathDesiredDistance(static_cast<float>(data["pathDesiredDistance"].as_double(8.0)));
    agent.setMaxSpeed(static_cast<float>(data["maxSpeed"].as_double(160.0)));
    agent.setAutoMove(data["autoMove"].as_bool(false));
...
Component* CreateUiCanvas(GameObject& owner)
{
    return owner.addComponent<UiCanvas>();
}
Component* CreateUiPanel(GameObject& owner)
{
    return owner.addComponent<UiPanel>();
}
Component* CreateUiLabel(GameObject& owner)
{
    return owner.addComponent<UiLabel>();
}
Component* CreateUiButton(GameObject& owner)
{
    return owner.addComponent<UiButton>();
}
Component* CreateUiCheckBox(GameObject& owner)
{
    return owner.addComponent<UiCheckBox>();
}
Component* CreateUiSlider(GameObject& owner)
{
    return owner.addComponent<UiSlider>();
}
...
void ReadSpriteBatch(Component& component, const ct::Json& data, Assets* assets)
{
    ...
    for (size_t i = 0; i < entries.size(); ++i)
    {
        ...
        const int index = batch.add(texture, ReadVec2(e["position"]), ReadVec2(e["size"], Math::Vec2(1.0f)), color);
        ...
        for (size_t i = 0; i < clips.size(); ++i)
        {
            ...
            if (assets)
                if (const ct::Json* tex = clip.find("texture"))
                    texture = assets->GetTexture(tex->as_cstr());
            anim.addClip(clip["name"].as_cstr(""), texture, (int)clip["frameWidth"].as_int(0),
                         (int)clip["frameHeight"].as_int(0), (int)clip["frameCount"].as_int(0),
...
const TypeEntry* AllEntries(std::size_t& count)
{
    static const TypeEntry kEntries[] = {
        ...
        {ComponentType::CapsuleShape, "CapsuleShape", &CreateCapsuleShape, &WriteCapsuleShape, &ReadCapsuleShape,
         nullptr},
        ...
        {ComponentType::Light, "DirectionalLight2D", &CreateDirectionalLight, &WriteDirectionalLight,
         &ReadDirectionalLight, &IsDirectionalLight},
...
const TypeEntry* MatchEntryIn(const TypeEntry* entries, std::size_t count, const Component& component,
                              const TypeEntry*& fallback)
{
    for (std::size_t i = 0; i < count; ++i)
    {
...
bool Serializer::RegisterType(ComponentType type, const char* name, CreateFn create, WriteFn write, ReadFn read,
                              MatchFn matches)
{
