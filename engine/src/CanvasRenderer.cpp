...
static const char* WATER_FRAGMENT_SHADER_SOURCE = R"(#version 300 es
...
uniform vec2 u_waterFlowB;
uniform vec4 u_waterTint;
uniform float u_waterHighlight;
...
static void UnpackColor(unsigned int packed, unsigned char& r, unsigned char& g, unsigned char& b, unsigned char& a)
{
    r = packed & 0xFF;
    g = (packed >> 8) & 0xFF;
    b = (packed >> 16) & 0xFF;
...
CanvasRenderer::CanvasRenderer()
    : mConfig(), mStats(), mVertices(), mIndices(), mDrawCalls(), mCurrentTextureId(0), mProjection(1.0f),
      ...
      mDirectionalLightShadowColorLoc(-1), mDirectionalShadowFilterLoc(-1), mLights(nullptr), mLightCount(0),
      mDirectionalLights(nullptr), mDirectionalLightCount(0), mShadowFlagsLoc(-1), mShadowColorLoc(-1),
      mShadowFilterLoc(-1), mShadowAtlasLoc(-1), mCanvasSizeLoc(-1), mOccluders(nullptr), mOccluderCount(0),
      mOrthoWidth(1280.0f), mOrthoHeight(720.0f)
{
...
void CanvasRenderer::DrawItems(const RenderItem* items, size_t count, const PointLight* lights, size_t lightCount,
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        for (size_t j = 0; j < item.commands.size(); ++j)
        {
            ...
            case RenderCommand::kRect:
            {
                ...
                EmitQuad(item.blendMode, c.textureId, c.normalTextureId, c.customProgram, c.water, m, c.width, c.height,
                         c.texWidth, c.texHeight, c.pivotX, c.pivotY, c.srcX, c.srcY, c.srcW, c.srcH,
            ...
            case RenderCommand::kPolygon:
            {
                ...
                if (points && points->size() >= 3)
                {
                    ...
                    EmitPolygon(item.blendMode, c.textureId, c.normalTextureId, c.customProgram, c.water, m, *points,
                                c.color, c.lightMask, c.texWidth, c.texHeight);
                }
                break;
            }
...
void CanvasRenderer::EmitQuad(BlendMode blendMode, unsigned int textureId, unsigned int normalTextureId,
                              unsigned int customProgram, const WaterEffect& water, const Matrix2D& matrix, float width,
                              float height, int texWidth, int texHeight, float pivotX, float pivotY, float srcX,
                              float srcY, float srcW, float srcH, bool flipX, bool flipY, const Color& color,
                              unsigned int lightMask)
{
    if (mVertices.size() + 4 > mConfig.maxVertices)
        Flush();

    if (mDrawCalls.empty() || mDrawCalls.back().textureId != textureId ||
        mDrawCalls.back().normalTextureId != normalTextureId || mDrawCalls.back().program != customProgram ||
        mDrawCalls.back().blendMode != blendMode || mDrawCalls.back().water != water)
    {
...
void CanvasRenderer::EmitPolygon(BlendMode blendMode, unsigned int textureId, unsigned int normalTextureId,
                                 unsigned int customProgram, const WaterEffect& water, const Matrix2D& matrix,
                              ...
                              const char* text, const Color& color, unsigned int lightMask)
{
    ...
    for (const char* character = text; *character; ++character)
    {
        ...
        if (code != ' ')
        {
            ...
            EmitQuad(blendMode, mFontTexture, 0, 0, WaterEffect(), matrix * Matrix2D::Translation(penX, penY), size,
                     size, FONT_ATLAS_W, FONT_ATLAS_H, 0.0f, 0.0f, srcX, srcY, 8.0f, 8.0f, false, false, color,
                     lightMask);
        }
        penX += size;
    }
...
void CanvasRenderer::Flush()
{
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, (GLsizeiptr)(mVertices.size() * sizeof(Vertex)), mVertices.data());
    mState.BindElementArrayBuffer(mIBO);
    glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, (GLsizeiptr)(mIndices.size() * sizeof(unsigned short)),
                    mIndices.data());
...
void CanvasRenderer::ApplyDrawCalls()
{
    ...
    for (size_t i = 0; i < mDrawCalls.size(); ++i)
    {
        ...
        // A caller supplied custom program deliberately has precedence
        // over the built-in material effect.
        const bool waterActive =
            call.program == 0 && call.water.enabled && call.normalTextureId != 0 && mWaterProgram != 0;
        GLuint wantProgram = (call.program != 0) ? call.program : (waterActive ? mWaterProgram : mProgram);
        if (wantProgram != mProgram)
        {
            mState.UseProgram(wantProgram);
            GLint mvpLoc = wantProgram == mWaterProgram ? mWaterMvpLoc : glGetUniformLocation(wantProgram, "u_mvp");
            if (mvpLoc >= 0)
...
bool CanvasRenderer::SetupShaders()
{
    ...
    mWaterProgram = CreateProgram(VERTEX_SHADER_SOURCE, WATER_FRAGMENT_SHADER_SOURCE);
    if (!mWaterProgram)
    {
        glDeleteProgram(mProgram);
        mProgram = 0;
    ...
    GLint waterTextureLoc = glGetUniformLocation(mWaterProgram, "u_texture");
    if (waterTextureLoc >= 0)
        glUniform1i(waterTextureLoc, 0);
    if (mWaterNormalMapLoc >= 0)
        glUniform1i(mWaterNormalMapLoc, 3);
    glUseProgram(0);
    return true;
}
...
void CanvasRenderer::SetupBuffers()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(mConfig.maxVertices * sizeof(Vertex)), nullptr, GL_STREAM_DRAW);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, (GLsizeiptr)(mConfig.maxVertices * 3 * sizeof(unsigned short)), nullptr,
                 GL_STREAM_DRAW);
...
void CanvasRenderer::SetupFontTexture()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, FONT_ATLAS_W, FONT_ATLAS_H, 0, GL_RGBA, GL_UNSIGNED_BYTE, atlas);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
void CanvasRenderer::RenderShadowAtlas()
{
    ...
    static const int textureSize = 2048;
    static const Math::Vec2 directions[4] = {Math::Vec2(1.0f, 0.0f), Math::Vec2(0.0f, 1.0f), Math::Vec2(-1.0f, 0.0f),
                                             Math::Vec2(0.0f, -1.0f)};
    ...
    for (int lightIndex = 0; lightIndex < mLightCount; ++lightIndex)
    {
        ...
        for (int sector = 0; sector < 4; ++sector)
        {
            ...
            {
                Math::Vec2 a =
                    Math::Vec2(mOccluderEdges[edgeIndex].x, mOccluderEdges[edgeIndex].y) - mLights[lightIndex].position;
                Math::Vec2 b =
                    Math::Vec2(mOccluderEdges[edgeIndex].z, mOccluderEdges[edgeIndex].w) - mLights[lightIndex].position;
            ...
            glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(mShadowVertices.size() * sizeof(ShadowVertex)),
    ...
    for (int lightIndex = 0; lightIndex < mDirectionalLightCount; ++lightIndex)
    {
        ...
        for (size_t edgeIndex = 0; edgeIndex < mOccluderEdges.size(); ++edgeIndex)
        {
            Math::Vec2 a = Math::Vec2(mOccluderEdges[edgeIndex].x, mOccluderEdges[edgeIndex].y) - canvasCenter;
            Math::Vec2 b = Math::Vec2(mOccluderEdges[edgeIndex].z, mOccluderEdges[edgeIndex].w) - canvasCenter;
