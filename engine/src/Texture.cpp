...
bool Texture::Update(const unsigned char* rgba)
{
    if (!mId || !rgba || mWidth <= 0 || mHeight <= 0)
        return false;
    glBindTexture(GL_TEXTURE_2D, mId);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mWidth, mHeight, GL_RGBA, GL_UNSIGNED_BYTE, rgba);
    glBindTexture(GL_TEXTURE_2D, 0);
    return true;
}
