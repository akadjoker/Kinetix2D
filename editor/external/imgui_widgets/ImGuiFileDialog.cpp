...
std::string BuildEntryLabel(const ImGuiFileDialog::Entry& entry)
{
    ...
    {
        prefix = use_icons ? "[D]" : nullptr;
    }
    else if (is_script)
    {
        prefix = use_icons ? "[C]" : nullptr;
    }
    else if (is_bytecode)
    {
        prefix = use_icons ? "[T]" : nullptr;
    }
    else
    {
        prefix = use_icons ? "[F]" : nullptr;
    }
...
void ImGuiFileDialog::ConfirmCreateFolder()
{
    if (!mNewFolderName.empty())
    {
        ...
        {
            std::fprintf(stderr, "ImGuiFileDialog: could not create folder '%s' (%s)\n",
