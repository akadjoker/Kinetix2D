...
AABB ComputeBodyAABB(const Body& body, const Transform& xf)
{
    AABB aabb = ComputeShapeAABB(body.Shapes()[0], xf);
    for (int i = 1; i < body.ShapeCount(); ++i)
    {
        const AABB shapeAabb = ComputeShapeAABB(body.Shapes()[i], xf);
        aabb.lowerBound = Min(aabb.lowerBound, shapeAabb.lowerBound);
        aabb.upperBound = Max(aabb.upperBound, shapeAabb.upperBound);
    }
    return aabb;
...
{
    return b->Type() == BodyType::Kinematic || (b->Type() == BodyType::Dynamic && b->IsAwake());
}
...
static Math::Vec2 ClosestPointOnSegment(const Math::Vec2& point, const Math::Vec2& a, const Math::Vec2& b)
{
    Math::Vec2 edge = b - a;
...
static Math::Vec2 ClosestPointOnShape(const Shape& shape, const Transform& xf, const Math::Vec2& point)
{
...
bool World::TestMotion(const Body& body, const Math::Vec2& motion, MotionResult& out, float safeMargin,
                       bool includeSensors) const
{
    ...
    for (size_t candidateIndex = 0; candidateIndex < candidates.size(); ++candidateIndex)
    {
        ...
        const Shape* otherShapes = other->Shapes();
        for (int selfIndex = 0; selfIndex < body.ShapeCount(); ++selfIndex)
        {
            const AABB selfSweep = ComputeShapeAABB(selfShapes[selfIndex], selfTransform);
            for (int otherIndex = 0; otherIndex < other->ShapeCount(); ++otherIndex)
            {
                const Shape& otherShape = otherShapes[otherIndex];
                if ((!includeSensors && (selfShapes[selfIndex].isSensor || otherShape.isSensor)) ||
                    !ShouldCollide(selfShapes[selfIndex].filter, otherShape.filter))
                    continue;

                const AABB otherAabb = ComputeShapeAABB(otherShape, otherTransform);
                ...
                if (!TestOverlap(pairSweep, otherAabb))
                    continue;

                ShapeCastInput cast;
                cast.shapeA = &otherShape;
                cast.transformA = otherTransform;
                cast.shapeB = &selfShapes[selfIndex];
                ...
                // A→B is the Box2D shape-cast normal. B is the moving
                // body, therefore it already points out of the obstacle.
                best = hit;
...
bool World::TestPosition(const Body& body, const Math::Vec2& position, MotionResult& out, bool includeSensors) const
{
    out = MotionResult();
    if (body.ShapeCount() == 0)
        return false;
    ...
    for (size_t candidateIndex = 0; candidateIndex < candidates.size(); ++candidateIndex)
    {
        ...
        for (int selfIndex = 0; selfIndex < body.ShapeCount(); ++selfIndex)
        {
            const AABB selfAabb = ComputeShapeAABB(selfShapes[selfIndex], selfTransform);
            for (int otherIndex = 0; otherIndex < other->ShapeCount(); ++otherIndex)
            {
                const Shape& otherShape = otherShapes[otherIndex];
                if ((!includeSensors && (selfShapes[selfIndex].isSensor || otherShape.isSensor)) ||
                ...
                Manifold manifold;
                bool flipped = false;
                if (!CollideShapePair(manifold, flipped, selfShapes[selfIndex], selfTransform, otherShape,
                                      otherTransform) ||
                ...
                if (flipped)
                {
                    worldManifold.Initialize(&manifold, otherTransform, ShapeRadius(otherShape), selfTransform,
                                             ShapeRadius(selfShapes[selfIndex]));
                    out.normal = worldManifold.normal;
                }
                else
                {
                    worldManifold.Initialize(&manifold, selfTransform, ShapeRadius(selfShapes[selfIndex]),
...
void World::RayCastGather(const Math::Vec2& origin, const Math::Vec2& translation, uint16_t categoryMask,
